...
body {
				...
				<table width=100%>
					...
					<tr>
						...
						<td colspan=3>
							...
							%> <a
							href="javascript:download('<%=bean.getFilename()%>')"><%=bean.getFilename()%></a>&nbsp;&nbsp;
							<font color=blue>(<%=bean.getFilesize()%> KBytes)
						</font> <%
 } else {
 %> 등록된 파일이 없습니다. <%
 }
 %>
					...
					<tr>
						<td align="center" height="250px">내용</td>
						<td colspan=4><%=bean.getContent()%></td>
					</tr>
					<tr>
						<td colspan=4 align=right><%=bean.getIp()%> 로부터 글을 남기셨습니다 /
							조회수 : <%=bean.getCount()%></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			<td align="center" colspan="2">
				...
				href="update.jsp?nowPage=<%=nowPage%>&num=<%=num%>">수정</a> | <a
				href="reply.jsp?nowPage=<%=nowPage%>">답변</a> | <a
				href="delete.jsp?nowPage=<%=nowPage%>&num=<%=num%>">삭제</a> ]
			</td>
		</tr>
	</table>
